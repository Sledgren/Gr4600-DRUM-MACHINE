...
const model = {
  ...
  reversed: Object.fromEntries(TRACKS.map(t => [t.id, false])),
  selectedPad: "kick",
  selectedTrack: "kick",
...
const els = {
  ...
  swingDivisionButtons: [...document.querySelectorAll("[data-swing-division]")],
...
function swingOffset(step) {
  const subdivisionsPerQuarter = stepSubdivision();
  const pairSteps = model.swingDivision === "1/8"
    ? subdivisionsPerQuarter
    : subdivisionsPerQuarter / 2;
  const swungPosition = pairSteps / 2;
  if (step % pairSteps !== swungPosition) return 0;
  const pairDuration = stepDuration() * pairSteps;
  return pairDuration * ((model.swing - 50) / 100);
}

function swingLabel(value = model.swing) {
  const factor = Math.round(Number(value ?? 50));
  if (factor === 50) return "50% STRAIGHT";
  return `${factor}% ${factor > 50 ? "LATE" : "EARLY"}`;
}

function swingAmountLabel() {
  const amount = Math.round(model.swing - 50);
  if (amount === 0) return "STRAIGHT";
  return `${amount > 0 ? "+" : ""}${amount}% ${amount > 0 ? "LATE" : "EARLY"}`;
}

function syncSwingDivisionButtons() {
  els.swingDivisionButtons.forEach(btn => {
    const active = btn.dataset.swingDivision === model.swingDivision;
...
function refreshScreen() {
  ...
  els.swingReadout.textContent = swingLabel();
  syncSwingDivisionButtons();
  ...
  if (els.sampleModeReadout) els.sampleModeReadout.textContent = model.sampleStretch ? "STRETCH" : sampleUsesPitchPreserve() ? "KEY LOCK" : "CLASSIC";
...
function resetSamplerControlsForNewFile() {
  stopSampleLoop();
  stopSampleChops();
  model.sampleRun = true;
  model.sampleStretch = false;
  model.sampleTimeStretch = true;
  model.sampleCutSelf = true;
  model.samplePitch = 0;
  model.tunes.sample = 0;
  model.sampleStart = 0;
  model.sampleEnd = 1;
  if (els.sampleTimeStretch) els.sampleTimeStretch.checked = true;
  if (els.sampleCutSelf) els.sampleCutSelf.checked = true;
}
...
function buildPadControls() {
  document.querySelectorAll(".pad-unit").forEach((unit, index) => {
    const padButton = unit.querySelector("[data-pad]");
    if (!padButton) return;
    const trackId = padButton.dataset.pad;
    const track = TRACKS.find(t => t.id === trackId);
    if (!track) return;
    unit.dataset.track = trackId;
    unit.style.setProperty("--track-color", track.color);
    if (unit.querySelector(".pad-control-deck")) return;
    const deck = document.createElement("div");
    deck.className = "pad-control-deck";
    deck.innerHTML = `
      <button class="pad-sound-chip" type="button" title="${model.soundNames[trackId] || track.label}">
        <i class="lane-led"></i><span class="pad-sound-name">${model.soundNames[trackId] || track.label}</span>
      </button>
      <div class="pad-control-row">
        <label><span>LVL</span><input class="pad-level" type="range" min="0" max="1" step="0.01" value="${model.volumes[trackId]}"></label>
        <label><span>PAN</span><input class="pad-pan" type="range" min="-1" max="1" step="0.01" value="${model.pans[trackId] || 0}"></label>
        <label><span>PIT</span><input class="pad-pitch" type="range" min="-12" max="12" step="1" value="${model.tunes[trackId] || 0}"></label>
      </div>
      <div class="pad-file-row">
        <select class="pad-kit-select" aria-label="${track.label} sound kit"><option value="">KIT</option></select>
        <button class="pad-file-btn" type="button">FILE</button>
        <input class="pad-file-input" type="file" accept="audio/*">
      </div>
    `;
    unit.insertBefore(deck, padButton);

    const soundChip = deck.querySelector(".pad-sound-chip");
    soundChip.addEventListener("click", async event => {
      event.stopPropagation();
      await unlockAudio();
      selectPad(trackId);
    ...
    soundChip.addEventListener("contextmenu", event => {
      event.preventDefault();
      selectPad(trackId);
      openPadControls(trackId);
      contextTarget = { type: "pad", trackId, step: model.step };
      showUnitMenu(event.clientX, event.clientY, "pad");
    });

    const level = deck.querySelector(".pad-level");
    level.addEventListener("input", event => {
      model.volumes[trackId] = Number(event.target.value);
      setFxScope(`${track.label} LEVEL`, `${Math.round(model.volumes[trackId] * 100)} · PAD MIX`, track.color);
      syncPadControls();
    });
    level.addEventListener("change", () => remember(`${track.label} LEVEL`));

    const pan = deck.querySelector(".pad-pan");
    pan.addEventListener("input", event => {
      model.pans[trackId] = Number(event.target.value);
      const label = model.pans[trackId] < -0.02 ? "LEFT" : model.pans[trackId] > 0.02 ? "RIGHT" : "CENTER";
      setFxScope(`${track.label} PAN`, label, track.color);
      syncPadControls();
    });
    pan.addEventListener("change", () => remember(`${track.label} PAN`));

    const pitch = deck.querySelector(".pad-pitch");
    pitch.addEventListener("input", event => {
      model.tunes[trackId] = Number(event.target.value);
      const hiddenPitch = document.querySelector(`.track[data-track="${trackId}"] .pitch-fader`);
      if (hiddenPitch) hiddenPitch.value = String(model.tunes[trackId]);
      setFxScope(`${track.label} PITCH`, `${model.tunes[trackId]} ST`, track.color);
      syncPadControls();
    });
    pitch.addEventListener("change", () => remember(`${track.label} PITCH`));

    const kitSelect = deck.querySelector(".pad-kit-select");
    populateKitSelect(kitSelect, trackId);
    kitSelect.addEventListener("change", () => {
      remember(`${track.label} SOUND`);
      if (kitSelect.value) loadKitSound(trackId, kitSelect.value);
    });
    kitSelect.addEventListener("mouseenter", () => {
      if (kitSelect.value) previewKitSound(kitSelect.value);
    });

    const fileBtn = deck.querySelector(".pad-file-btn");
    const fileInput = deck.querySelector(".pad-file-input");
    ...
    fileInput.addEventListener("change", event => {
      loadTrackSound(trackId, event.target.files[0], fileBtn);
      event.target.value = "";
    });

    unit.addEventListener("contextmenu", event => {
      if (event.target.closest(".step-menu, .unit-menu")) return;
      event.preventDefault();
      selectPad(trackId);
      openPadControls(trackId);
      contextTarget = { type: "pad", trackId, step: model.step };
...
function getPlaybackBuffer(ctx, buffer, reversed = false) {
  if (!buffer || !reversed) return buffer;
  const cached = reversedBufferCache.get(buffer);
  if (cached) return cached;
  const flipped = ctx.createBuffer(buffer.numberOfChannels, buffer.length, buffer.sampleRate);
  for (let channel = 0; channel < buffer.numberOfChannels; channel++) {
    const src = buffer.getChannelData(channel);
    const dst = flipped.getChannelData(channel);
    for (let i = 0, j = src.length - 1; i < src.length; i++, j--) dst[i] = src[j];
  }
  reversedBufferCache.set(buffer, flipped);
  return flipped;
}
...
function updateTrackSoundLabels(trackId) {
  ...
  document.querySelectorAll(`.pad-unit[data-track="${trackId}"] .pad-sound-name`).forEach(label => {
    label.textContent = name;
    label.title = name;
  });
...
function refreshKitSelects() {
  ...
  document.querySelectorAll(".pad-unit").forEach(padEl => {
    populateKitSelect(padEl.querySelector(".pad-kit-select"), padEl.dataset.track);
  });
}
...
function closeSoundBrowser() {
  if (!els.soundBrowser || els.soundBrowser.hidden) return;
  els.soundBrowser.hidden = true;
  els.soundBrowser.classList.remove("piano-mode");
  setInfo("SOUND BROWSER CLOSED");
}
...
function playKitBuffer(ctx, out, buffer, t, vol, activeTrack = "") {
  ...
  const playbackBuffer = getPlaybackBuffer(ctx, buffer, !!model.reversed[activeTrack]);
  ...
  const stopAt = t + Math.min(playbackBuffer.duration * Math.max(0.18, decay), hardCap);
...
function playKitBufferPitch(ctx, out, buffer, t, vol, activeTrack = "", semitones = 0) {
  ...
  const stopAt = t + Math.min(playbackBuffer.duration / Math.max(0.25, src.playbackRate.value), 2.5);
  src.start(t);
  src.stop(stopAt + 0.02);
  return src;
}
...
function mixerFxChain(ctx, destination, trackId, t = ctx.currentTime) {
  const output = trackOutputDestination(ctx, destination, trackId);
  if (renderingDryStems) {
    const dryInput = ctx.createGain();
    dryInput.connect(output);
    return dryInput;
  }
...
function trackOutputDestination(ctx, destination, trackId) {
  const panValue = Math.max(-1, Math.min(1, Number(model.pans?.[trackId] || 0)));
  if (!panValue || !ctx.createStereoPanner) return destination;
...
function playSample(t, ctx = ensureAudio(), out = masterGain, vol = model.volumes.sample) {
  ...
  if (sampleUsesPitchPreserve()) {
    return playGranularSample(t, ctx, out, vol);
  }
  const { buffer, region } = samplePlaybackState(ctx);
  const src = ctx.createBufferSource();
  const g = outputGain(ctx, t, vol, out);
  src.buffer = buffer;
  src.playbackRate.value = samplePlaybackRate() * samplePitchRate();
...
function playSampleChop(index = 0, t = ensureAudio().currentTime) {
  ...
  src.buffer = buffer;
  src.playbackRate.value = sampleUsesPitchPreserve() ? samplePitchRate() : samplePlaybackRate() * samplePitchRate();
  src.connect(mixerFxChain(ctx, g, "sample", t));
...
function sampleKeyShiftActive() {
  return Math.round(Number(model.samplePitch || 0)) !== 0 || Math.round(Number(model.tunes.sample || 0)) !== 0;
}

function sampleUsesPitchPreserve() {
  return !!(model.sampleStretch || model.sampleTimeStretch || sampleKeyShiftActive());
}
...
function samplePlaybackState(ctx) {
  const buffer = getPlaybackBuffer(ctx, sampleBuffer, !!model.reversed.sample);
  const region = sampleRegion();
  if (!model.reversed.sample) return { buffer, region };
  const start = Math.max(0, sampleBuffer.duration - region.end);
  const end = Math.min(sampleBuffer.duration, sampleBuffer.duration - region.start);
  return { buffer, region: { start, end, duration: Math.max(0.03, end - start) } };
}
...
function playGranularSample(t, ctx = ensureAudio(), out = masterGain, vol = model.volumes.sample) {
  ...
  for (let elapsed = 0; elapsed < duration; elapsed += hop) {
    ...
    src.start(start, offset, Math.min(grain * pitchRate, buffer.duration - offset));
...
function startSampleLoop(t = audioCtx.currentTime) {
  ...
  const { buffer, region } = samplePlaybackState(audioCtx);
  sampleSource.buffer = buffer;
  sampleSource.loop = true;
  sampleSource.loopStart = region.start;
  sampleSource.loopEnd = region.end;
  sampleSource.playbackRate.value = samplePlaybackRate() * samplePitchRate();
...
function clearProject(keepDrums = true) {
  ...
  TRACKS.forEach(track => { model.reversed[track.id] = false; });
  TRACKS.forEach(track => { model.pans[track.id] = 0; });
  model.swing = 50;
  model.swingDivision = "1/16";
...
function snapshot() {
  return {
    tempo: model.tempo,
    swing: model.swing,
    swingDivision: model.swingDivision,
    master: model.master,
...
async function restore(data) {
  ...
  model.swing = Math.max(29, Math.min(71, Number(data.swing ?? 50)));
  model.swingDivision = data.swingDivision === "1/8" ? "1/8" : "1/16";
  ...
  model.pans = { ...model.pans, ...(data.pans || {}) };
  ...
  model.reversed = { ...model.reversed, ...(data.reversed || {}) };
...
function restorePatternSnapshot(entry) {
  ...
  if (entry.swing !== undefined) model.swing = Math.max(29, Math.min(71, Number(entry.swing)));
  model.swingDivision = entry.swingDivision === "1/8" ? "1/8" : "1/16";
  TRACKS.forEach(track => {
...
async function decodeSampleDataUrl(dataUrl, name, options = {}) {
  await unlockAudio();
  if (options.resetControls) {
    resetSamplerControlsForNewFile();
    sampleBuffer = null;
    model.sampleName = "";
    model.sampleDataUrl = "";
    if (els.sampleName) els.sampleName.textContent = "Loading sample...";
    drawWave();
  }
  const res = await fetch(dataUrl);
  const arr = await res.arrayBuffer();
...
async function loadSample(file) {
  if (!file) return;
  remember("SAMPLE LOAD");
  const dataUrl = await readFileAsDataUrl(file);
  await decodeSampleDataUrl(dataUrl, file.name.replace(/\.[^.]+$/, ""), { resetControls: true });
...
async function loadFactoryKit() {
  try {
    const ctx = ensureAudio();
    const manifest = await fetch("assets/kit-manifest.json?v=1052").then(r => r.json());
...
function renderTrack(trackId, lengthSec, sampleRate, options = {}) {
  ...
  try {
    ...
    if (trackId === "sample" && sampleBuffer && model.sampleRun) {
      if (sampleUsesPitchPreserve()) {
        const { region } = samplePlaybackState(off);
        const loopDur = model.sampleStretch ? Math.max(model.length * stepSec, 0.1) : Math.max(region.duration, 0.1);
        for (let t = 0; t < model.length * stepSec; t += loopDur) {
          playGranularSample(t, off, out, model.volumes.sample);
        }
        return off.startRendering();
      }
      const src = off.createBufferSource();
      const g = off.createGain();
      const { buffer, region } = samplePlaybackState(off);
      src.buffer = buffer;
      src.loop = true;
...
function bindUi() {
  window._mainKnobs = {
    ...
    swing: mountKnob(els.swingKnob, {
      label: "SWING", min: 29, max: 71, value: model.swing, unit: "",
      onChange: value => {
        model.swing = Math.round(value);
        els.swing.value = String(model.swing);
        setInfo(`SWING ${swingAmountLabel()} · ${model.swingDivision}`);
        refreshScreen();
      }
    }),
  ...
  els.swingDivisionButtons.forEach(btn => {
    btn.addEventListener("click", () => {
      model.swingDivision = btn.dataset.swingDivision === "1/8" ? "1/8" : "1/16";
      syncSwingDivisionButtons();
      setInfo(`SWING ${model.swingDivision} · ${swingAmountLabel()}`);
      setFxScope("SWING", `${model.swingDivision} ${swingAmountLabel()}`, "#ff7417");
    });
  });
  ...
  els.sampleInput.addEventListener("change", async event => {
    await loadSample(event.target.files[0]);
    event.target.value = "";
  });
  ...
  bindReadyCollapses();
  bindSoundEqCanvas();
  els.soundBrowserClose?.addEventListener("click", closeSoundBrowser);
  els.soundEq?.querySelectorAll("[data-eq-param]").forEach(input => {
  ...
  window.addEventListener("click", event => {
    if (!els.soundBrowser || els.soundBrowser.hidden) return;
    if (event.target.closest(".sound-browser, .unit-menu, .step-menu, .pattern-menu")) return;
    closeSoundBrowser();
  });
  ...
  const handleGlobalKeydown = async event => {
    if (event.gr4600Handled) return;
    const tag = event.target && event.target.tagName;
    const typingTarget = tag === "INPUT" || tag === "TEXTAREA" || tag === "SELECT" || event.target?.isContentEditable;
    if (event.key === "Escape" && els.soundBrowser && !els.soundBrowser.hidden) {
      event.gr4600Handled = true;
      event.preventDefault();
      closeSoundBrowser();
      return;
    }
    if (event.code === "Space" || event.key === " " || event.key === "Spacebar") {
      if (typingTarget || event.repeat) return;
      event.gr4600Handled = true;
      event.preventDefault();
      event.stopPropagation();
      await unlockAudio();
...
function syncPadSelection() {
  ...
  document.querySelectorAll(".pad-unit").forEach(unit => {
    const id = unit.dataset.track;
    unit.classList.toggle("selected", id === model.selectedPad);
    unit.classList.toggle("muted", !!model.muted[id]);
    unit.classList.toggle("reversed", !!model.reversed[id]);
  });
  syncPadControls();
}

function openPadControls(trackId) {
  document.querySelectorAll(".pad-unit").forEach(unit => {
    unit.classList.toggle("expanded", unit.dataset.track === trackId);
  });
}

function syncPadControls() {
  document.querySelectorAll(".pad-unit").forEach((unit, index) => {
    const trackId = unit.dataset.track;
    if (!trackId) return;
    const track = TRACKS.find(t => t.id === trackId);
    const name = model.soundNames[trackId] || track?.label || `PAD ${index + 1}`;
    unit.style.setProperty("--track-color", track?.color || "#00a8ff");
    unit.querySelector(".pad-sound-name")?.replaceChildren(document.createTextNode(name));
    const level = unit.querySelector(".pad-level");
    const pan = unit.querySelector(".pad-pan");
    const pitch = unit.querySelector(".pad-pitch");
    if (level && document.activeElement !== level) level.value = String(model.volumes[trackId] ?? 0.7);
    if (pan && document.activeElement !== pan) pan.value = String(model.pans[trackId] ?? 0);
    if (pitch && document.activeElement !== pitch) pitch.value = String(model.tunes[trackId] ?? 0);
    unit.style.setProperty("--pad-level", String(Math.round((model.volumes[trackId] ?? 0.7) * 100)));
    unit.style.setProperty("--pad-pan", String(Math.round((model.pans[trackId] ?? 0) * 100)));
    unit.style.setProperty("--pad-pitch", String(model.tunes[trackId] ?? 0));
    unit.querySelector(".pad-sound-chip")?.classList.toggle("off", !!model.muted[trackId]);
  });
}
...
async function applyUnitAction(action) {
  ...
  if (action === "mute") {
    ...
    remember(`${trackId.toUpperCase()} REVERSE`);
    model.reversed[trackId] = !model.reversed[trackId];
    if (trackId === "sample" && model.playing && model.sampleRun) startSampleLoop();
    syncPadSelection();
    setInfo(`${trackId.toUpperCase()} REVERSE ${model.reversed[trackId] ? "ON" : "OFF"}`);
    setFxScope(`${trackId.toUpperCase()} REVERSE`, model.reversed[trackId] ? "BACKWARD PLAYBACK" : "FORWARD PLAYBACK", TRACKS.find(t => t.id === trackId)?.color || "#7edbff");
  } else if (action === "browser") {
    openSoundBrowser(trackId);
  } else if (action === "piano-roll") {
    openPianoRoll(trackId);
  } else if (action === "browse-file") {
    const fileInput = document.querySelector(`.pad-unit[data-track="${trackId}"] .pad-file-input`)
      || document.querySelector(`.track[data-track="${trackId}"] .sound-file`);
    fileInput?.click();
